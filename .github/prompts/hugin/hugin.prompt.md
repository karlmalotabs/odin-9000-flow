...
11. **Overlaying a node on a sibling (e.g. an icon centered on a placeholder shape) requires `layoutPositioning: 'ABSOLUTE'`** on that node before setting its `x`/`y` — inside an Auto Layout parent, manual `x`/`y` on a node still in normal (`'AUTO'`) flow is silently ignored and it stacks as the next flow item instead.
